...
export default function EstablishmentItemPage() {
  ...
  useEffect(() => {
    ...
    const loadMetrics = async () => {
      try {
        ...
        setMetrics(
          Object.fromEntries(
            rows.map((row) => [String(row.id), Number(row.total_views || 0)])
          )
        );
      } catch (error) {
  ...
  const enrichedItems = useMemo(
    () => items.map((item) => ({
      ...item,
      total_views: metrics[String(item.id)] ?? Number(item.total_views || 0),
    })),
    [items, metrics]
  );

  const insights = useMemo(() => {
    const totalViews = enrichedItems.reduce(
          ...
          <>
            ...
            {count === 0 ? (
              <div className="item-manager__empty">
                ...
                <p>Cadastre o primeiro item para começar a aparecer no catálogo público e nas descobertas locais.</p>
                <GlobalButton
                  variant="success"
