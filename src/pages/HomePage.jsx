...
const getCompanyImages = (establishment) => {
  ...
  return [
    ...
    establishment?.cover,
    files.find((file) =>
      ["background", "cover", "banner"].includes(String(file?.type || "").toLowerCase())
    )?.public_url,
...
function Rail({ title, subtitle, children, railRef }) {
  ...
  return (
    <section className="hp-discovery-section">
      ...
      <div className="hp-discovery-rail" ref={railRef}>
        {children}
      </div>
    </section>
  );
...
export default function HomePage() {
  ...
  useEffect(() => {
    ...
    const loadDiscovery = async () => {
      try {
        ...
        const { data: ecosystemData } = await api.get(`${apiV1BaseUrl}/directory`, {
          params: {
            city: city || undefined,
            uf: uf || undefined,
            limit: 100,
        ...
        setDiscovery({
          establishments: Array.isArray(ecosystemData?.establishments)
            ? ecosystemData.establishments
            : [],
  ...
  const locationLabel = useMemo(() => {
    ...
    return parts.length
      ? parts.join(" - ")
      : "em todos os catálogos disponíveis";
  }, [discovery.city, discovery.uf]);

  const openCompany = (establishment) => {
    if (!establishment?.slug) return;
    navigate(`/establishment/view/${establishment.slug}`);
  };

  const handleCompanyKeyDown = (event, establishment) => {
    if (event.key === "Enter" || event.key === " ") {
      event.preventDefault();
      openCompany(establishment);
    }
  };
      ...
      <main className="hp-wrapper">
        <section className="hp-hero">
          <div className="hp-hero__copy">
            ...
            <p>
              A Nexus prioriza empresas próximas sem limitar a descoberta: explore
              catálogos da sua cidade, de outras regiões e de lugares que você
              pretende visitar.
            </p>
            <div className="hp-hero__actions">
              <button
                type="button"
                className="hp-primary-action"
            ...
            <div className="hp-benefits">
              ...
              <span>Explore outras cidades</span>
              <span>QR Code e link</span>
            </div>
          </div>
        ...
        <section className="hp-discovery" aria-labelledby="hp-discovery-title">
          <div className="hp-discovery-intro">
            <div>
              ...
              <p>
                Empresas cadastradas no ecossistema Peter Tecnet aparecem aqui
                quando o catálogo está habilitado para a Nexus. A localização
                apenas organiza a prioridade dos resultados.
              </p>
            </div>
            <div className="hp-location-chip">
              <FaMapMarkerAlt /> {locationLabel}
            </div>
          </div>

          {discoveryLoading && (
            <div className="hp-discovery-skeleton">
          ...
          {!discoveryLoading &&
            !discoveryError &&
            discovery.establishments.length > 0 && (
              <Rail
                title="Empresas"
              ...
              >
                {discovery.establishments.slice(0, 24).map((establishment) => {
                  const companyName =
                    establishment.fantasy || establishment.name || "Empresa";
                  const sourceApp =
                    establishment?.source_app?.name || establishment?.app?.name;

                  return (
                    <article
                      className="hp-company-card"
                      key={establishment.id}
                      onClick={() => openCompany(establishment)}
                      onKeyDown={(event) => handleCompanyKeyDown(event, establishment)}
                      role="button"
                      tabIndex={0}
                      aria-label={`Conhecer ${companyName}`}
                    >
                      <EntityImage
                      ...
                      <div>
                        ...
                        <small>
                          {sourceApp ? `${sourceApp} · ` : ""}
                          {Number(establishment.total_views || 0).toLocaleString("pt-BR")} visualizações
                        </small>
                      </div>
                    </article>
                  );
          ...
          {!discoveryLoading && !discoveryError && discovery.items.length > 0 && (
            <Rail
              ...
              subtitle="Produtos e serviços dos catálogos disponíveis na Nexus."
              railRef={itemsRail}
            >
                ...
                return (
                  ...
                  >
                    ...
                    <div className="hp-item-card__body">
                      <span className="hp-item-card__company">
                      ...
                      <div className="hp-item-card__meta">
                        ...
                        <span>
                          <FaEye />{" "}
                          {Number(item.total_views || 0).toLocaleString("pt-BR")}
                        </span>
                      </div>
                    </div>
                  </article>
          ...
          {!discoveryLoading &&
            ...
            discovery.items.length === 0 && (
              <div className="hp-discovery-state">
                ...
                <span>
                  Assim que houver catálogos habilitados para a Nexus eles
                  aparecerão aqui.
                </span>
              </div>
            )}
        ...
        <section className="hp-content" aria-labelledby="hp-how-title">
          <div className="hp-section-intro">
            ...
            <p>
              Cadastre a empresa, organize os itens, publique e acompanhe o
              interesse do público pelas visualizações.
            </p>
          </div>
        </section>
      </main>
