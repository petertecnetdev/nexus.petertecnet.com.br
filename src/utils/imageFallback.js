...
function normalizeFallbackLabel(value) {
  ...
  const cleaned = raw
    .replace(
      /^(?:imagem|image|foto|photo|avatar|logo|icone|ícone)(?:\s+(?:de|do|da|dos|das))?\s*/i,
      ""
    )
    .replace(/\s+/g, " ")
    .trim();
...
function findContextLabel(image) {
  ...
  return (
    getCandidateLabel(textNode) ||
    normalizeFallbackLabel(textNode.textContent || "")
  );
}
...
function applyInitialsFallback(image, failedSrc) {
  ...
  if (image.dataset && image.dataset.imageFallbackApplied === "true" && isFallbackDataUri(currentSrc)) {
    return;
  }
  ...
  if (
    image.dataset &&
    !image.dataset.imageFallbackOriginalSrc &&
    failedSrc &&
    !isFallbackDataUri(failedSrc)
  ) {
...
function inspectImage(image) {
  ...
  if (image.complete && Number(image.naturalWidth || 0) === 0) {
    applyInitialsFallback(image, currentSrc);
  }
}
...
export function installGlobalImageFallbacks() {
  ...
  const handleError = (event) => {
    ...
    window.setTimeout(() => {
      ...
      if (failedSrc && currentSrc && currentSrc !== failedSrc && !isFallbackDataUri(currentSrc)) {
        return;
      }
