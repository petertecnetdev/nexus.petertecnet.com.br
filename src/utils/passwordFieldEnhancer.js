...
const ensureStyles = () => {
  ...
  style.textContent = `
    ...
    .pt-password-toggle {
      position: absolute; right: .45rem; top: 50%; transform: translateY(-50%); z-index: 6;
      width: 2.35rem; height: 2.35rem; min-width: 2.35rem; padding: 0; display: grid; place-items: center;
      border: 1px solid rgba(148,163,184,.28); border-radius: .7rem; background: rgba(15,23,42,.78);
      color: inherit; cursor: pointer; line-height: 1; box-shadow: none;
    }
...
const existingToggle = (host) => host.querySelector([
  `[${GENERATED_TOGGLE_ATTR}]`,
...
const enhanceInput = (input) => {
  ...
  const updateCaps = (event) => {
    caps.hidden = !Boolean(event.getModifierState?.('CapsLock'));
  };
  ...
  host.addEventListener('focusout', () => {
    window.setTimeout(() => {
      if (!host.contains(document.activeElement)) setHidden(input, toggle);
    }, 0);
  });
