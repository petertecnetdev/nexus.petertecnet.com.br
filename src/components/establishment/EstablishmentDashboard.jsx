...
export default function EstablishmentDashboard({ establishment, navigate, onDelete, deleting, onActivate, activating }) {
  ...
  const openPresentation = () => {
    ...
    navigate(`/establishment/view/${catalogSlug}`);
  };
  ...
  return (
    <article className={`company-card ${catalogActive ? "is-catalog-active" : "is-catalog-available"}`} data-card-kind="establishment">
      <button type="button" className="company-card__visual" onClick={openPresentation} aria-label={catalogActive ? `Ver apresentação de ${name}` : `${name} ainda não possui catálogo Nexus`} disabled={!catalogActive}>
      ...
      <div className="company-card__body">
        ...
        <div className="company-card__title-row">
          ...
          {catalogActive && <button type="button" className="company-card__quick-open" onClick={openPresentation} aria-label="Ver apresentação da empresa" title="Ver apresentação"><i className="fas fa-arrow-up-right-from-square" aria-hidden="true" /></button>}
        </div>
