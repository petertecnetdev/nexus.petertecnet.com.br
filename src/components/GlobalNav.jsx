...
import { FaBuilding, FaSearch, FaShoppingCart, FaTimes } from "react-icons/fa";
...
export default function GlobalNav({ loadingMenu, handleLogout }) {
  ...
  const goToResult = (path) => { closeSearch(); setSearch(""); navigate(path); };
  ...
  useEffect(() => {
    ...
    if (term.length < 2) { setResults({ companies: [], items: [] }); setSearching(false); return undefined; }
    ...
    const timer = window.setTimeout(async () => {
      try {
        ...
        setSearchOpen(true);
      } catch (error) { if (error?.code !== "ERR_CANCELED") setResults({ companies: [], items: [] }); }
  ...
  const onLogout = async () => {
    ...
    finally { localStorage.removeItem("token"); closeUserMenu(); window.dispatchEvent(new Event("authChanged")); window.location.replace("/login"); }
  };
  ...
  return <header className="globalnav"><div className="globalnav__bar">
    <div className="globalnav__left"><Link to="/" className="globalnav__brand" aria-label="Ir para o início da Nexus" title="Nexus"><img src="/images/logo.png" alt="Nexus" className="globalnav__logo" /></Link><nav className="globalnav__links" aria-label="Navegação principal">{publicNavigation.map((item) => <NavLink key={item.path} to={item.path} end={item.end} className={({ isActive }) => ["globalnav__link", isActive ? "globalnav__link--active" : ""].filter(Boolean).join(" ")}>{item.label}</NavLink>)}</nav></div>

    <div className="globalnav__searchWrap" ref={searchRef}><div className="globalnav__searchBox"><FaSearch aria-hidden="true" /><input value={search} onFocus={() => search.trim().length >= 2 && setSearchOpen(true)} onChange={(event) => setSearch(event.target.value)} placeholder="Pesquisar empresas, produtos, serviços..." aria-label="Pesquisar na Nexus" autoComplete="off" />{search && <button type="button" className="globalnav__searchClear" onClick={() => { setSearch(""); closeSearch(); }} aria-label="Limpar pesquisa"><FaTimes /></button>}</div>{searchOpen && search.trim().length >= 2 && <div className="globalnav__searchResults"><div className="globalnav__searchTop"><strong>Resultados</strong><span>{searching ? "Buscando…" : `${results.companies.length + results.items.length} encontrados`}</span></div>{!searching && !hasResults && <div className="globalnav__searchEmpty">Nenhum resultado para “{search.trim()}”.</div>}{results.companies.length > 0 && <div className="globalnav__searchGroup"><small>Empresas</small>{results.companies.map((company) => <button type="button" key={`company-${company.id}`} onClick={() => goToResult(`/catalog/${company.slug}`)}><FaBuilding /><span><strong>{company.fantasy || company.name}</strong><em>{[company.category, company.city, company.uf].filter(Boolean).join(" · ")}</em></span></button>)}</div>}{results.items.length > 0 && <div className="globalnav__searchGroup"><small>Itens</small>{results.items.map((item) => <button type="button" key={`item-${item.id}`} onClick={() => goToResult(`/item/view/${item.slug}`)}><FaBoxOpen /><span><strong>{item.name}</strong><em>{[item.type, item.category, item.establishment?.fantasy || item.establishment?.name].filter(Boolean).join(" · ")}</em></span></button>)}</div>}</div>}</div>
