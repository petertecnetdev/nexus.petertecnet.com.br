...
function loadTelemetry(apiBaseUrl, appSlug) {
  ...
  telemetryPromise = new Promise((resolve, reject) => {
    ...
    if (existing) { if (window.PeterTecnetTelemetry) ready(); else { existing.addEventListener("load", ready, { once: true }); existing.addEventListener("error", () => reject(new Error("Não foi possível carregar a telemetria Peter Tecnet.")), { once: true }); } return; }
...
function loadSdk() {
  ...
  sdkPromise = new Promise((resolve, reject) => {
    ...
    if (existing) { if (customElements.get("peter-ecosystem-launcher")) resolve(); else { existing.addEventListener("load", () => resolve(), { once: true }); existing.addEventListener("error", () => reject(new Error("Não foi possível carregar o Peter Tecnet Ecosystem SDK.")), { once: true }); } return; }
    const script = document.createElement("script"); script.src = SDK_URL; script.async = true; script.dataset.peterEcosystemSdk = SDK_VERSION; script.addEventListener("load", () => resolve(), { once: true }); script.addEventListener("error", () => reject(new Error("Não foi possível carregar o Peter Tecnet Ecosystem SDK.")), { once: true }); document.head.appendChild(script);
  });
  return sdkPromise;
}
...
function dockLauncherInNavbar(launcher) {
  ...
  const applyDockedLayout = () => {
    ...
    const shell = launcher.shadowRoot.querySelector(".launcher"); const button = launcher.shadowRoot.querySelector(".launcher-button"); const panel = launcher.shadowRoot.querySelector(".panel");
  ...
  const mount = () => {
    ...
    const toggle = target.querySelector?.(".navbar-toggler"); if (toggle && toggle.parentElement === target) target.insertBefore(launcher, toggle); else if (launcher.parentElement !== target) target.appendChild(launcher);
    Object.assign(launcher.style, { display: "inline-flex", alignItems: "center", marginLeft: "8px", flex: "0 0 auto" }); launcher.setAttribute("data-peter-navbar-docked", "true"); applyDockedLayout(); return true;
  };
  let frame = 0;
  const scheduleMount = () => { if (frame) return; frame = window.requestAnimationFrame(() => { frame = 0; mount(); applyDockedLayout(); }); };
  mount();
  const shadowObserver = new MutationObserver(applyDockedLayout); if (launcher.shadowRoot) shadowObserver.observe(launcher.shadowRoot, { childList: true, subtree: true });
  const navObserver = new MutationObserver(scheduleMount); navObserver.observe(document.body, { childList: true, subtree: true }); window.addEventListener("resize", scheduleMount, { passive: true });
  return () => { shadowObserver.disconnect(); navObserver.disconnect(); window.removeEventListener("resize", scheduleMount); if (frame) window.cancelAnimationFrame(frame); };
}

export default function PeterAccountGateway({ apiBaseUrl, appSlug, children }) {
  const hostRef = useRef(null);
  useEffect(() => {
    let active = true; let launcher = null; let cleanupDock = null; const host = hostRef.current; const api = apiBaseUrl || "https://api.petertecnet.com.br/api";
    loadTelemetry(api, appSlug || "").catch((error) => console.error("[Peter Tecnet Telemetry]", error)).finally(() => loadSdk().then(() => {
      if (!active || !host) return; launcher = document.createElement("peter-ecosystem-launcher"); launcher.setAttribute("api-base", api); launcher.setAttribute("app-slug", appSlug || ""); launcher.setAttribute("sdk-version", SDK_VERSION); host.replaceChildren(launcher); cleanupDock = dockLauncherInNavbar(launcher);
    }).catch((error) => console.error("[Peter Tecnet Ecosystem]", error)));
    return () => { active = false; cleanupDock?.(); launcher?.remove(); host?.replaceChildren(); };
  }, [apiBaseUrl, appSlug]);
