...
const resolveItemImage = (item) => item?.image_url || item?.image || item?.avatar || item?.images?.cover || item?.images?.main || item?.images?.avatar || (Array.isArray(item?.images?.gallery) ? item.images.gallery[0] : null) || firstFileUrl(item?.files) || null;
...
const normalizeEstablishment = (establishment) => {
  ...
  return {
    ...
    images: {
      ...
      background: establishment?.images?.background || establishment.background || backgroundFile || null,
    },
  };
};
...
export default function useEstablishmentItemsByIdentifier(identifier) {
  ...
  const fetchItems = useCallback(async () => {
    if (!identifier) {
      setEstablishment(null); setItems([]); setApiError(null); setLoading(false); return;
    }

    setLoading(true); setApiError(null);
    try {
      const { data } = await api.get(`${apiV1BaseUrl}/catalog/${encodeURIComponent(identifier)}`);
      const payload = data?.data || data || {};
      const resolvedEstablishment = normalizeEstablishment(payload?.establishment || null);
      ...
      setItems(resolvedItems.map((item) => normalizeItem(item, resolvedEstablishment)));
    } catch (error) {
      setApiError(getApiMessage(error, error?.message || "Erro ao carregar o catálogo."));
      setItems([]); setEstablishment(null);
    } finally {
      setLoading(false);
    }
  }, [identifier]);
  ...
  return { establishment, items, count: items.length, loading, apiError, reload: fetchItems };
}
