...
import {
  collectEstablishmentMediaFiles,
...
const normalizeEstablishment = (establishment) => {
  ...
  const images =
    rawImages && !Array.isArray(rawImages) && typeof rawImages === "object"
      ? rawImages
      : {};
  const files = collectEstablishmentMediaFiles(establishment);
  const logo = resolveEstablishmentLogo(establishment, files);
  const background = resolveEstablishmentBackground(establishment, files, logo);
